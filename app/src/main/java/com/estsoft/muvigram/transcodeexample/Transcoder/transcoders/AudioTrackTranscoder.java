package com.estsoft.muvigram.transcodeexample.Transcoder.transcoders;
...
import com.estsoft.muvigram.transcodeexample.Transcoder.audiosurface.AudioChannel;
...
public class AudioTrackTranscoder implements TrackTranscoder {
    ...
    public boolean stepPipeline() {
        ...
        do {
            staus = drainDecoder(0);
            if (staus != DRAIN_STATE_NONE) busy = true;
            // NOTE: not repeating to keep from deadlock when encoder is full.
        } while (staus == DRAIN_STATE_SHOULD_RETRY_IMMEDIATELY);
