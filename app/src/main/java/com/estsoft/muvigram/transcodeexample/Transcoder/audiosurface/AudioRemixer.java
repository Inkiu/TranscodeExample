...
public interface AudioRemixer {
    ...
    AudioRemixer DOWNMIX = new AudioRemixer() {
        ...
        @Override
        public void remix(final ShortBuffer inSBuff, final ShortBuffer outSBuff, final float volume) {
            // TODO volume control
            ...
            for (int i = 0; i < samplesToBeProcessed; ++i) {
                ...
                outSBuff.put( (short) ( (m - SIGNED_SHORT_LIMIT) * volume) );
            }
        }
    };
    ...
    AudioRemixer UPMIX = new AudioRemixer() {
        ...
        public void remix(final ShortBuffer inSBuff, final ShortBuffer outSBuff, final float volume) {
            ...
            final int samplesToBeProcessed = Math.min(inRemaining, outSpace);
            for (int i = 0; i < samplesToBeProcessed; ++i) {
                final short inSample = (short)(inSBuff.get() * volume);
    ...
    AudioRemixer PASSTHROUGH = new AudioRemixer() {
        ...
        public void remix(final ShortBuffer inSBuff, final ShortBuffer outSBuff, final float volume) {
            // Passthrough
            for ( int i = inSBuff.position() ; i < inSBuff.limit() ; i ++ ) {
                outSBuff.put( (short)(inSBuff.get() * volume) );
            }
